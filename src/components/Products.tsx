...
const products = [{
  ...
  achievements: ["Led team of 12 (devs, AI engineers, QA, design) and mentored 2 Product Analysts", "Launched 0-to-1 revenue stream, generating 5M INR in first year", "Scaled AI-driven self check-in/out and Ticketing & Routing solution for 20 hotels", "Deployed patient registration and voice navigation for 10 hospitals", "Oversaw Payment gateway and other API integrations for seamless user experience", "Enabled 30k+ user interactions per day across multiple industries"],
  metrics: [{
    icon: TrendingUp,
    label: "5M INR",
    sublabel: "First Year Revenue"
  }, {
    icon: Users,
    label: "30k+",
    sublabel: "Daily Interactions"
  }, {
    icon: Zap,
    label: "30+",
    sublabel: "Enterprise Deployments"
  }],
  ...
  achievements: ["Improved 12-week user retention from 60% to 85%", "Implemented gamified rewards system", "Redesigned onboarding journeys", "Revamped in-app support & feedback system", "Enhanced company website for better SEO and lead generation"],
  metrics: [{
    icon: TrendingUp,
    label: "85%",
  ...
  achievements: ["Onboarded McDonald's and Haldiram's as enterprise clients", "Generated 4M INR in ARR and Processed 10M+ lifetime orders", "Introduced Payment gateway integrations for Bifrost platform", "Led development of Fine Dine Suite with kitchen dashboards and waiter app", "Reduced table turnaround time by 20%", "Improved delivery metrics from 97% to 98.2% using data-based ranking algorithms"],
  metrics: [{
    icon: TrendingUp,
    label: "4M INR",
  ...
  achievements: ["Increased refurbished cars/month by 10x", "Reduced turnaround time (TAT) by 37%", "Improved inspection accuracy with image & sound processing algorithms", "Drove 15% operational cost reduction through automation", "Created dashboards & analytics tools for data-driven decisions"],
  metrics: [{
    icon: TrendingUp,
    label: "10x",
...
const Products = () => {
  return <section id="products" className="py-24 bg-background">
      <div className="container mx-auto px-6">
        <h2 id="products-heading" className="font-['Space_Grotesk'] text-4xl md:text-5xl font-bold text-foreground mb-4 text-center">Product Work Experience</h2>
        ...
        <div className="space-y-8 max-w-5xl mx-auto">
          ...
          opacity: 0
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 p-4 bg-background/50 rounded-lg">
                {product.metrics.map((metric, idx) => <div key={idx} className="flex items-center gap-3">
              ...
              <div className="mb-6">
                ...
                <ul className="space-y-2">
                  {product.achievements.map((achievement, idx) => <li key={idx} className="text-muted-foreground flex items-start">
              ...
              <div className="flex flex-wrap gap-2">
                {product.tags.map((tag, tagIndex) => <Badge key={tagIndex} variant="secondary" className="bg-accent/10 text-accent hover:bg-accent/20">
                    {tag}
                  </Badge>)}
              </div>
            </Card>)}
