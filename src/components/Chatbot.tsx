...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Hi! I'm Sohit's AI assistant. Ask me anything about his experience, skills, or background!" }
  ]);
  ...
  const sendMessageWithText = async (text: string) => {
    ...
    try {
      const response = await fetch(CHAT_URL, {
        ...
        body: JSON.stringify({ messages: [...messages, userMessage].slice(1) }), // Exclude initial greeting
      });
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 h-24 w-24 rounded-full bg-accent hover:bg-accent/90 shadow-lg shadow-accent/25 flex items-center justify-center transition-transform hover:scale-105 overflow-hidden ${!isOpen ? 'animate-subtle-bounce' : ''}`}
      >
        {isOpen ? (
          <X size={32} className="text-accent-foreground" />
        ) : (
      ...
      {/* Chat Window */}
      {isOpen && (
        <Card className="fixed bottom-24 right-6 z-50 w-[350px] md:w-[400px] h-[500px] flex flex-col border-accent/30 shadow-xl shadow-accent/10 animate-fade-in">
          {/* Header */}
          ...
          <form onSubmit={sendMessage} className="p-4 border-t border-border">
            <div className="flex gap-2">
              <Input
                ...
                disabled={isLoading}
                className="flex-1 bg-secondary border-border focus-visible:ring-accent"
              />
              <Button
                type="submit"
                size="icon"
