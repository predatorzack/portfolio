...
const Chatbot = () => {
  ...
  return <>
    ...
    <button 
      ...
      className={`fixed bottom-6 right-6 z-50 h-24 w-24 rounded-full bg-accent hover:bg-accent/90 shadow-lg shadow-accent/25 flex items-center justify-center transition-transform hover:scale-105 overflow-hidden ${!isOpen ? 'animate-subtle-bounce animate-glow' : ''}`}
    >
          ...
          <div className="px-4 pb-2 flex flex-wrap gap-2">
            {QUICK_REPLIES.map((question, index) => (
              <button 
                ...
                className="text-xs px-3 py-1.5 rounded-full border border-accent/30 hover:bg-accent/10 transition-colors text-accent"
              >
                {question}
              </button>
            ))}
